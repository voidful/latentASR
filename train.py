...
def main() -> None:
    ...
    best_wer = float("inf")
    if model.use_latent:
        ckpt_prefix = "latent_qwen_asr"
        best_metric_name = "latent"
    elif cfg.use_prompt_tuning:
        ckpt_prefix = "prompt_tuning_qwen_asr"
        best_metric_name = "prompt_tuning"
    elif cfg.use_lora_r16:
        ckpt_prefix = "lora_r16_qwen_asr"
        best_metric_name = "lora_r16"
    else:
        ckpt_prefix = "baseline_qwen_asr"
        ...
        for batch in pbar:
            ...
            if model.use_latent:
                ...
                if baseline_ce is not None and "predicted_value" in stats and "lr_ce" in stats:
                    predicted_value = stats["predicted_value"].float().view(-1)  # (B,)
                    ...
                    with torch.no_grad():
                        ...
                        p_neg = 0.3  # 30% chance of forced negative target
                        is_neg_sample = torch.rand(1).item() < p_neg
                        
                        acc_diff = lr_acc - baseline_acc  # (B,), range ~[-0.3, 0.3]
                        
                        # Fallback: when both accuracies are 0 (extreme OOD), acc_diff is always 0.
                        # In this case, use CE difference as surrogate signal.
                        both_zero = (baseline_acc.abs() < 1e-6) & (lr_acc.abs() < 1e-6)  # (B,)
                        if both_zero.all():
                            ce_diff = (baseline_ce - lr_ce_per_sample).clamp(-2.0, 2.0)  # (B,)
                            target_value = torch.tanh(ce_diff * 0.5).view(-1)
                        else:
                            ...
                            target_value = torch.tanh(acc_diff * 3.0).view(-1)
                    ...
                    l_value = F.mse_loss(predicted_value, target_value)
                    ...
                    if global_step % 5 == 0:
                        ...
                        print(f"\n[Value-DEBUG] step={global_step} "
                              ...
                              f"pred={predicted_value.mean().item():.4f} l_value={l_value.item():.4f}")
                else:
